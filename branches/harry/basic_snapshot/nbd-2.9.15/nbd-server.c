 ...
 */
/* Include LFS defines, which defines behaviours of some of the
 * following headers, so must come before those
 */
#include "lfs.h"
...
typedef struct {
	...
	u32 *difmap;	     /**<  comment on the global difmap for this one */

    /* add by kanghua */
    char  *uri;                           /* in shm mem for parent child process comm */ 
    HLFS_CTRL * storage_ctrl;
    CTRL_REGION_T * ctrl_region;             /* in shm mem for parent child process comm  */
//    NBD_OPS_STAT *stat;             /* in shm mem for parent child process comm  */
} CLIENT;
...
int append_serve(SERVER *s, GArray *a)
{
	...
	if(e == 0) {
		for (rp = ai; rp != NULL; rp = rp->ai_next) {
            ...
            //HLOG_DEBUG("dup server addr:%p",s);
			ns = dup_serve (s);
            //HLOG_DEBUG("dupped server addr:%s",ns->storage_address);
			if (ns) {
...
int expread(off_t a, char *buf, size_t len, CLIENT *client) {
    ...
    HLOG_DEBUG("expread -- offset:%llu,len:%d",a,len);
#if 0
	off_t rdlen, offset;
...
#endif
    if(len != hlfs_read(client->storage_ctrl,buf,len,a)){
       HLOG_ERROR("read failed");
       g_assert(0);
       return -1;
    }
...
int expwrite(off_t a, char *buf, size_t len, CLIENT *client) {
	HLOG_DEBUG("enter func %s", __func__);
    HLOG_DEBUG("expwrite -- offset:%llu,len:%d",a,len);
#if 0
...
#endif
    if(len != hlfs_write(client->storage_ctrl,buf,len,a)){
       HLOG_ERROR("write failed");
       g_assert(0);
       return -1;
    }
	HLOG_DEBUG("leave func %s", __func__);
...
void negotiate(CLIENT *client) {
    ...
    hlfs_set_user_ctrl_region(client->storage_ctrl,client->ctrl_region);
#if 0 
    printf("init shm for ctrl by client\n");
    CTRL_REGION_T * _region = (CTRL_REGION_T*)mmap(NULL,sizeof(CTRL_REGION_T),PROT_READ|PROT_WRITE,MAP_SHARED|MAP_ANONYMOUS,-1,0);
    if(_region == NULL){
       printf("Failed to creat share mem for ctrl region\n");
       err("Failed/6:%m\n");
    }
    client->storage_ctrl->ctrl_region = _region;
#endif
    //client->storage_ctrl->ctrl_region = client->ctrl_region;
    if(0!=hlfs_open(client->storage_ctrl,1)){
        HLOG_ERROR("Failed to connect storage :%s",client->uri);
        err("Failed/5:%m\n");
    }
#if 1
    HLFS_STAT_T stat;
    if(0!=hlfs_stat(client->storage_ctrl,&stat)){
        HLOG_ERROR("Failed to connect storage :%s",client->uri);
        err("Failed/6:%m\n");
    }
	size_host = htonll((u64)(stat.max_fs_size));
	if (write(client->net, &size_host, 8) < 0)
        err("Negotiation failed: %m");
#endif 
    if (write(client->net, zeros, 124) < 0)
        err("Negotiation failed: %m");
    HLOG_DEBUG("negotiate succ !!!");
}
	...
	while (go_on) {
        ...
        if (request.type==NBD_CMD_DISC) {
            ...
            if (client->server->flags & F_COPYONWRITE) { 
                ...
                DEBUG("to deinit hlfs");
                deinit_hlfs(client->storage_ctrl);
            }
            go_on=FALSE;
            continue;
        }
...
#if 1
    GSList *clientslist = NULL;
    int nread = 0;
    int offset = 0;
#endif
    for(;;) {
        char msg_buf_r[512];
        memset(&msg_buf_r,0,512);
        offset = 0;
    _loop:   
        nread=read(g_msg_r_fd,msg_buf_r+offset,sizeof(NBD_OPS_CMD_T));
        offset += nread;
        if(nread > 0 && offset < sizeof(NBD_OPS_CMD_T)){
            HLOG_DEBUG("---- offset is %d, buf:%s",offset,msg_buf_r);
            goto _loop;
        }
        if(offset == sizeof(NBD_OPS_CMD_T)){
            HLOG_DEBUG("read from FIFO_R");
            NBD_OPS_CMD_T * nbd_cmd = (NBD_OPS_CMD_T*)(msg_buf_r);
            HLOG_DEBUG("receive msg .....type:%d fsname:%s value%d",
			    nbd_cmd->nbd_ops_cmd,nbd_cmd->uri,nbd_cmd->value);
            char* _name = nbd_cmd->uri;
            /*  find uri's client */
            int i;
            CLIENT *_client = NULL;
            for(i = 0;i<g_slist_length(clientslist);i++){
                HLOG_DEBUG("i:%d",i);
                _client = (CLIENT*)g_slist_nth_data(clientslist,i);               
                HLOG_DEBUG("_client : %p",_client);
                if(0 == strcmp(_client->uri,nbd_cmd->uri)){
                    HLOG_DEBUG("find it :%s",_client->uri);
                    break;
                }
            }
            ...
            nbd_rsp.err_no = 0;
            if(_client == NULL){
                HLOG_DEBUG("not find this fsname:%s",_name);
                nbd_rsp.err_no = 1;
                HLOG_DEBUG("write respons");
                write(g_msg_w_fd,&nbd_rsp,sizeof(NBD_OPS_RSP_T));
            }else{
                gint *_is_start_clean  = &_client->ctrl_region->is_start_clean;
                gint *_copy_waterlevel = &_client->ctrl_region->copy_waterlevel;
                switch (nbd_cmd->nbd_ops_cmd) { 
                    case start_clean:
                        if(nbd_cmd->value == 0)
                            hlfs_clean_stop(_client->storage_ctrl);
                        else if (nbd_cmd->value == 1)
                            hlfs_clean_start(_client->storage_ctrl);
                        HLOG_DEBUG("set start clean :%d",nbd_cmd->value);
                        write(g_msg_w_fd,&nbd_rsp,sizeof(NBD_OPS_RSP_T));
                        break;
                    case set_copy_waterlevel:
                       hlfs_set_clean_level(_client->storage_ctrl,nbd_cmd->value);
                        HLOG_DEBUG("set copy waterlevel :%d",nbd_cmd->value);
                        write(g_msg_w_fd,&nbd_rsp,sizeof(NBD_OPS_RSP_T));
                        break;
                    case query_stat:
                        {
                         NBD_OPS_STAT stat;
                         stat.is_start_clean   = *_is_start_clean; 
                         stat.copy_waterlevel = *_copy_waterlevel;
                         write(g_msg_w_fd,&nbd_rsp,sizeof(NBD_OPS_RSP_T));
                         write(g_msg_w_fd,&stat,sizeof(NBD_OPS_STAT));
                        }
                        break;                                            
                    default:
                        ;
        ...
        timeout.tv_sec = 1;
        timeout.tv_usec = 0;
        if(select(max+1, &rset, NULL, NULL, &timeout)>0) {
            DEBUG("accept, ");
            for(i=0;i<servers->len;i++) {
                ...
                if(FD_ISSET(serve->socket, &rset)) {
                    ...
                    client->server=serve;
                    HLOG_DEBUG("77dbg server addr 2:%p",client->server);
...
#if 1 
                    HLOG_DEBUG("init shm for fsname field");
                    char *name = (char*)mmap(NULL,128,PROT_READ|PROT_WRITE,MAP_SHARED|MAP_ANONYMOUS,-1,0);
                    if(name == NULL){
                        HLOG_ERROR("Failed to create share mem for fsname");
                        continue;
                    }
                    HLOG_DEBUG("init shm for ctrl by client");
                    CTRL_REGION_T * region = (CTRL_REGION_T*)mmap(NULL,sizeof(CTRL_REGION_T),PROT_READ|PROT_WRITE,MAP_SHARED|MAP_ANONYMOUS,-1,0);
                    if(region == NULL){
                        HLOG_ERROR("Failed to create share mem for ctrl region");
                        continue;
                    }
#if 0
                    NBD_OPS_STAT *qs = (NBD_OPS_STAT *)mmap(NULL,sizeof(NBD_OPS_STAT),PROT_READ|PROT_WRITE,MAP_SHARED|MAP_ANONYMOUS,-1,0);
                    if(qs == NULL){
                        printf("Failed to create share mem for query stat\n");
                        continue;
                    }
		    client->stat = qs;
#endif
                    client->ctrl_region = region;
                    client->uri = name;
                    HLOG_DEBUG("put client :%p to clients ......",client);
                    clientslist = g_slist_append(clientslist,client);
#endif 
...
void setup_servers(GArray* servers) {
	...
	for(i=0;i<servers->len;i++) {
        HLOG_DEBUG("server:%p",&(g_array_index(servers,SERVER,i)));
                                  ...
                                  gpointer user_data)
{
    ...
    syslog(level, "syslog is %s", message);
}
#endif

int create_msg_fifo(){
    int  _pid  = getpid();
    if (-1 == mkdir("/tmp/nbd-0-", 0777)) {
	    HLOG_FATAL("mkdir /tmp/nbd-0- error!");
	    exit(-1);
    }
    if (-1 == mkdir("/tmp/nbd-1-", 0777)) {
	    HLOG_FATAL("mkdir /tmp/nbd-1- error!");
	    exit(-1);
    }
    snprintf(G_FIFO_R,128,"%s/%s/%d","/tmp", "nbd-0-",_pid);
    snprintf(G_FIFO_W,128,"%s/%s/%d","/tmp", "nbd-1-",_pid);
    HLOG_DEBUG("--FIFO-R :%s",G_FIFO_R);
    HLOG_DEBUG("--FIFO-W :%s",G_FIFO_W);

    if(mkfifo(G_FIFO_W,O_RDWR|O_NONBLOCK|0666)){
       HLOG_FATAL("cannot create fifoserver %s",G_FIFO_W);
       exit(-1);
    }
    if(mkfifo(G_FIFO_R,O_RDWR|O_NONBLOCK|0666)){
       HLOG_FATAL("cannot create fifoserver %s",G_FIFO_R);
       exit(-1);
    }

    HLOG_DEBUG("Preparing for writing bytes.....");
    g_msg_w_fd = open(G_FIFO_W,O_RDWR|O_NONBLOCK,0);
    if(g_msg_w_fd == -1){
       HLOG_FATAL("open   error:   %s",   strerror(errno));   
       HLOG_FATAL("open msg_fd failed %d",g_msg_w_fd); 
       exit(-1);
    }
    HLOG_DEBUG("Preparing for reading bytes.....");
    g_msg_r_fd = open(G_FIFO_R,O_RDWR|O_NONBLOCK,0);
    if(g_msg_r_fd == -1){
       HLOG_FATAL("open   error:   %s",   strerror(errno));   
       HLOG_FATAL("open msg_fd failed %d",g_msg_r_fd); 
       exit(-1);
    }
    return 0;  
}

int destroy_msg_fifo(){
	close(g_msg_r_fd);
	close(g_msg_w_fd);
    g_remove(G_FIFO_R);
    g_remove(G_FIFO_W);
    if (-1 == rmdir("/tmp/nbd-0-")) {
	    HLOG_FATAL("rmdir /tmp/nbd-0- error!");
	    exit(-1);
    }
    if (-1 == rmdir("/tmp/nbd-1-")) {
	    HLOG_FATAL("rmdir /tmp/nbd-1- error!");
	    exit(-1);
    }
    return 0;
}
...
int main(int argc, char *argv[]) {
	...
	/* Check HLOG */
	HLOG_INFO("Hlfs log begin for hlfs");
	if (sizeof( struct nbd_request )!=28) {
		HLOG_ERROR("Bad size of structure. Alignment problems?");
		exit(EXIT_FAILURE) ;
	}
	...
	serve=cmdline(argc, argv);
 	...
 	HLOG_DEBUG("77 dbg server from cmdline:%p", serve);
	servers = parse_cfile(config_file_pos, &err);
	
	if(serve) {
		serve->socket_family = AF_UNSPEC;
		append_serve(serve, servers);
 	HLOG_DEBUG("77 dbg server from cmdline:%d", serve->port);
		if (!(serve->port)) {
			CLIENT *client;
#ifndef ISSERVER
			...
			client=g_malloc(sizeof(CLIENT));
			client->server=serve;
 			HLOG_DEBUG("77 dbg server addr:%p", client->server);
			client->net=0;
			client->exportsize=OFFT_MAX;
    ...
    if(!servers || !servers->len) {
		HLOG_WARN("Could not parse config file: %s", err ? err->message : "Unknown error");
	}
    
	if((!serve) && (!servers||!servers->len)) {
		HLOG_INFO("Nothing to do! Bye!");
		exit(EXIT_FAILURE);
	}
#if 0
	daemonize(serve);
