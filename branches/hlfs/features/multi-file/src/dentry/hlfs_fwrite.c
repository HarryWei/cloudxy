...
/*
   ...
   3, Write datas into this file.
 */

int hlfs_fwrite(struct hlfs_ctrl *ctrl, const char *f_path, 
				char *write_buf, uint32_t write_len, 
				uint64_t pos) {
	g_message("9999 enter func %s", __func__);
    if(ctrl == NULL || f_path ==NULL || NULL == write_buf){
