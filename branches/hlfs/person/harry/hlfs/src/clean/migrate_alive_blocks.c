...
int migrate_alive_blocks (struct hlfs_ctrl *hctrl, SEG_USAGE_T *seg_usage){
	...
	char *content = NULL;
	uint32_t size;
	if (0 != (ret = file_get_contents(hctrl->storage, \
					segfile, &content, &size))) {
	...
	for (i = 0;i < seg_usage->log_num;i++) {
		...
		//HLOG_DEBUG("bitmap:%hhx\n,bit:%d is %hhx", seg_usage->bitmap[idx], \
		i, seg_usage->bitmap[idx] & (i << (i % 8)));
		if (seg_usage->bitmap[idx] & (1 << (i % sizeof(gint)))) {
			...
			for (j = 0;j < lh->db_num; j++) {
				uint64_t _last_inode_write_timestamp = \
								       hctrl->last_write_timestamp;
				...
				if(1 == hctrl->is_compress) {
					...
					db_mine_storage_addr_offset = offset + \
								      LOG_HEADER_LENGTH + j * hctrl->sb.block_size;
				}
				set_offset(&db_mine_storage_addr, \
						db_mine_storage_addr_offset);
				set_segno (&db_mine_storage_addr, \
						seg_usage->segno);
				uint64_t db_cur_storage_addr = \
							       get_db_storage_addr_in_inode(hctrl->storage, \
									       &hctrl->inode, lh->start_db_no + j, \
									       hctrl->sb.block_size, hctrl->is_compress);
				HLOG_DEBUG("db:%u's mine storage addr:%llu, \
						cur storage addr:%llu", lh->start_db_no + i, \
						db_mine_storage_addr, db_cur_storage_addr);
				if(db_mine_storage_addr == \
						db_cur_storage_addr){
					HLOG_DEBUG("this is alive data block");
					HLOG_DEBUG("copy! ....... ");
					HLOG_DEBUG(" log[%d]  need move block:[%d]", \
							i, j);
					if(db_start == -1){
				...
				if (db_mine_storage_addr != \
						db_cur_storage_addr || j == lh->db_num - 1) {
					HLOG_DEBUG("if need ,append it ... ");
					if(db_start == -1) {
						HLOG_DEBUG\
							("do not need to copy this log ....... ");
						continue;
					}	
#if 0
					...
					uint32_t size; 
					size = append_log(hctrl, lh->data + (db_start \
								- lh->start_db_no) * hctrl->sb.block_size, \
							(uint32_t) db_start, (uint32_t) db_end);
					g_assert(size > 0);
					hctrl->last_offset += size;
#else
					char *db_buff = lh->data + (db_start - \
							lh->start_db_no) * hctrl->sb.block_size;
					g_mutex_lock  (hctrl->hlfs_access_mutex);
					if(_last_inode_write_timestamp != \
							hctrl->last_write_timestamp){
						HLOG_INFO("inode maybe has change, \
								for safe reason,redo check again");
						i--;
						g_mutex_unlock (hctrl->hlfs_access_mutex);
						continue;
					}	
					int size = append_log(hctrl, db_buff, db_start,\
							db_end, 0);
					g_mutex_unlock (hctrl->hlfs_access_mutex);
					if (size < 0) {
