...
#if 0
int dump_seg_usage_text(struct back_storage *storage, const \
		char *segment_usage_file, const char *seg_usage_text){
	HLOG_DEBUG("enter func %s", __func__);
	...
	if (0 > (ret = storage->bs_file_append(storage, file, \
					seg_usage_text, len))) {
...
int dump_segment_delmark(struct back_storage *storage, const char *segment_delmark_file, uint32_t segno){
	/*  TODO -- do not create runtime */
	HLOG_DEBUG("enter func %s", __func__);
	HLOG_DEBUG( "enter func %s, seg del mark file:%s", __func__, \
			segment_delmark_file);
	int ret = 0;
	char segtextbuf[128];
	uint32_t len;
	bs_file_t file = NULL;
	...
	file = storage->bs_file_open(storage, segment_delmark_file, \
			BS_WRITEABLE);
	if (file == NULL) {
...
int seg_usage4text(SEG_USAGE_T *seg_usage, const char *textbuf){
	...
	HLOG_DEBUG("segno str:%s,up sname:%s, inode addr:%s, log num:%s, \
			block num:%s, alive block str:%s", _segno_str, _up_sname, \
			_inode_addr_str, _log_num_str, _block_num_str,\
			_alive_block_str);
	...
	g_strfreev(v);
	HLOG_DEBUG("segno:%u,alive_blocks:%u,timestamp:%llu, log_num:%u, \
			block_num:%u", seg_usage->segno, \
			seg_usage->alive_block_num, \
			seg_usage->timestamp, seg_usage->log_num, \
			seg_usage->block_num);
	seg_usage->bitmap = (char*)g_malloc0((seg_usage->log_num - 1) / \
			sizeof(gint)  + 1);
...
int get_refer_inode_between_snapshots(struct back_storage *storage,uint32_t \
		segno,GList *snapshot_sorted_list, struct inode **inode, char **up_sname){
	...
	for (i = 1;i < num_entries;i++) {
		HLOG_DEBUG("idx:%d,pre snapshot inode segno:%u, \
				name:%s;cur snapshot inode segno:%u,name:%s", i, \
				get_segno(pre_snapshot->inode_addr), pre_snapshot->sname, \
				get_segno(cur_snapshot->inode_addr), cur_snapshot->sname);	
		if (segno > get_segno (pre_snapshot->inode_addr) && \
				segno < get_segno(cur_snapshot->inode_addr) ) {
			*inode = load_inode(storage, cur_snapshot->inode_addr);
			if (*inode == NULL) {
				HLOG_DEBUG("load inode failed");
				return -1;
		...
		pre_snapshot = cur_snapshot;
		cur_snapshot = (struct snapshot *)g_list_nth_data \
			       (snapshot_sorted_list, i);
	}
...
int load_all_seg_usage(struct back_storage *storage, \
	...
	HLOG_DEBUG("read seg usage file size:%u, \
			content len:%d", size, strlen(content));
#if 1
	gchar **segs = g_strsplit(content, "\n",0);
	HLOG_DEBUG("there are %d segno in segment usage file", \
			g_strv_length(segs));
	int i;
	for (i = 0;i < g_strv_length(segs) - 1;i++) {
		...
		if(seg_usage->alive_block_num != 0) {
			g_hash_table_insert(seg_usage_hashtable, \
					GINT_TO_POINTER((uint32_t) seg_usage->segno), seg_usage);
		}else{
			HLOG_DEBUG("segno:%llu can been delete ", seg_usage->segno);
			g_free(seg_usage);
		}
#endif
		SEG_USAGE_T *_seg_usage = g_hash_table_lookup(seg_usage_hashtable, \
				GINT_TO_POINTER((uint32_t)seg_usage->segno));
		if(_seg_usage != NULL) {
			HLOG_DEBUG("seg usage has exit,replace it");
			g_hash_table_replace(seg_usage_hashtable, \
					GINT_TO_POINTER((uint32_t) seg_usage->segno), seg_usage);
