...
extern "C" {
	...
	int local_file_append(struct back_storage *storage, bs_file_t file, \
			const char *write_buff, uint32_t write_len);
	int local_file_pread(struct back_storage *storage, bs_file_t file, \
	...
	bs_file_info_t *local_file_info(struct back_storage *storage, \
			const char *path);
	bs_file_info_t *local_list_dir(struct back_storage *storage, \
...
static struct back_storage *get_local_storage_ops(void){
	struct back_storage *storage = \
				       (struct back_storage*)malloc(sizeof(struct back_storage));
	...
	storage->bs_fs_rmfs = local_fs_rmfs,
