...
extern "C" {
	...
	int load_all_seg_usage  (struct back_storage *storage, \
			const char *seg_usage_file,  GHashTable* seg_usage_hashtable);
	int sort_all_seg_usage(GHashTable *ss_hashtable, GList *seg_usage_list);
	int get_refer_inode_between_snapshots(struct back_storage *storage, \
			uint32_t segno,GList *snapshot_sorted_list, struct inode **inode, \
			char **up_sname);
