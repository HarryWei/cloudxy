...
extern "C" {
	...
	int hlfs_open_by_snapshot(struct hlfs_ctrl *ctrl, const char *snapshot, \
			int flag);
	/**
	 ...
	 */
	int hlfs_find_inode_before_time(const char *uri, uint64_t timestamp, \
			uint64_t *inode_addr);
	 ...
	 * @return value: 0 is right, -1 is wrong
	 */
	int hlfs_get_inode_info(const char *uri, uint64_t inode_addr, \
			uint64_t *ctime, uint64_t *length);
