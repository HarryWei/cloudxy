...
int hlfs_read(struct hlfs_ctrl *ctrl, char *read_buf, uint32_t read_len, \
		uint64_t pos)
{
	...
	HLOG_INFO("Hlfs Read Req pos:%llu, read_len:%d, \
	...
	if ( pos % BLOCKSIZE != 0 ) {
		...
		memcpy(read_buf, first_block + pos % BLOCKSIZE, \
