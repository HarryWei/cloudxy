...
int cache_insert_blocks(CACHE_CTRL *cache_ctrl, uint32_t start_block_no, uint32_t block_count,char *block_buf)
{
    ...
    if(block_count + cache_ctrl->cache_size - get_cache_free_size(cache_ctrl) 
       ...
       HLOG_DEBUG("--need trigger flush worker to free cache block_count:%d,cache_size:%d,free_block:%d,min blocks:%d--",
                  block_count,cache_ctrl->cache_size,get_cache_free_size(cache_ctrl),(cache_ctrl->flush_trigger_level * cache_ctrl->cache_size)/100);
