...
int hdfs_file_append(struct back_storage *storage, bs_file_t file,\
		const char *write_buff, uint32_t write_len){
	//HLOG_DEBUG("hdfs -- enter func %s", __func__);
	hdfsFile hfile = (hdfsFile)file;
	//HLOG_DEBUG("append hdfs file:%p, write_len:%d", hfile, write_len);
	//HLOG_DEBUG("hdfs -- leave func %s", __func__);
	return hdfsWrite((hdfsFS)storage->fs_handler, \
			hfile, write_buff, write_len);
}
...
bs_file_info_t *hdfs_file_info(struct back_storage *storage, const char *path){
	...
	build_hdfs_path(full_path, storage->dir, storage->fs_name, path);
	hdfsFileInfo *hinfo = \
			      hdfsGetPathInfo((hdfsFS)storage->fs_handler, full_path);
	if (NULL == hinfo) {
		//HLOG_ERROR("hdfsGetPathInfo error");
		return NULL;
	}
	bs_file_info_t *info = \
			       (bs_file_info_t*)g_malloc0(sizeof(bs_file_info_t));
	if (NULL == info) {
...
static bs_file_t  __hlfs_file_open(struct back_storage *storage, \
	...
	//HLOG_DEBUG("hdfs full path %s",full_path);
	hdfsFile file = NULL;
	file = hdfsOpenFile((hdfsFS)storage->fs_handler, \
			full_path, flags, 0, 0, 0);
	if (NULL == file) {
...
bs_file_t hdfs_file_open(struct back_storage *storage, \
		const char *path, int flags){
	//HLOG_DEBUG("hdfs -- enter func %s", __func__);
	int  _flags = 0;
