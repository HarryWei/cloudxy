		...
		uint32_t flush_once_size)
{
	...
	//		flush_trigger_level:%llu, flush_once_size:%llu,%s", \
	...
	cache_ctrl->flush_worker = \
				   g_thread_create((GThreadFunc)flush_work, cache_ctrl, TRUE, NULL);
...
err:
	...
	if (cache_ctrl->block_cache) {
		while (g_trash_stack_height(&cache_ctrl->block_cache) != 0) {
			block_t *_block = \
					  (block_t *)g_trash_stack_pop(&cache_ctrl->block_cache);
