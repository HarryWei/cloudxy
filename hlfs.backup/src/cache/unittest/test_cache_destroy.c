...
void test_cache_destroy()
{
	...
	for (i = 0; i < HASH_NUM; i++) {
		block_t *_block = \
				  g_trash_stack_pop(&fixture.cache_ctrl->block_cache);
		g_hash_table_insert(fixture.cache_ctrl->block_map, \
				(gpointer)&i, (gpointer)_block);
		g_message("inserting key: %llu, value: %p", i, _block);
		g_queue_push_tail(fixture.cache_ctrl->dirty_block, \
				(gpointer)_block);
	}
...
int main(int argc, char **argv){
	...
	g_test_add("/misc/cache_destroy", Fixture, NULL, case_setup, \
			test_cache_destroy, case_teardown);
	return g_test_run();
}
