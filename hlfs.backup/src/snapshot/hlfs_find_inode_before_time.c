...
get_iaddr_bytime_in_seg(struct back_storage *storage, 
	...
	int count = storage->bs_file_pread(storage, file, \
			tmp_buf, _SEGMENT_SIZE, 0);
	if (0 > count) {
	...
	while (offset < count) {
		...
		imap = (struct inode_map_entry *) (tmp_buf + offset + \
				lh->log_size - sizeof(struct inode_map_entry));
		//g_message("%s -- This inode addr is %llu", __func__, \
		imap->inode_addr);
		inode = (struct inode *) (tmp_buf + offset + lh->log_size \
				- sizeof(struct inode_map_entry) - sizeof(struct inode));
		//g_message("%s -- This inode's mtime is %llu", __func__, \
		inode->mtime);
		//*inode_addr = imap->inode_addr;
		if ((0 == first_inode_flag) && (timestamp < inode->mtime)) {
			HLOG_ERROR("We can not find the inode addr before the \
					timestamp");
			ret = -1;
			goto out;
		}
		...
		if (timestamp < inode->mtime) {
			struct inode_map_entry *pre_imap = \
							   (struct inode_map_entry *) (tmp_buf + offset - \
