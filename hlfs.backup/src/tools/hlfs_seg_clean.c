...
static GOptionEntry entries[] = {
	...
	{"start segno to be move", 's', 0, G_OPTION_ARG_INT, &start_segno, \
		"start segno to be moved", "START SEGNO"},
	{"end   segno to be move", 'e', 0, G_OPTION_ARG_INT, &end_segno, \
		"start segno to be moved", "END SEGNO"},
	{"copy waterlevel ", 'w', 0, G_OPTION_ARG_INT, &copy_waterlevel, \
...
int main(int argc, char *argv[])
{
	...
	ret = load_all_seg_usage(ctrl->storage, SEGMENTS_USAGE_FILE, \
			seg_usage_hashtable);
	g_assert(ret == 0);
	...
	for (i = start_segno; i <= end_segno; i++) {
		...
		if (seg_usage != NULL) {
			...
			if (seg_usage->alive_block_num == 0) {
				...
				if (0 == ctrl->storage->bs_file_is_exist(ctrl->storage, \
							segfile)) {
					ret = ctrl->storage->bs_file_delete(ctrl->storage, \
							segfile);	  
					g_assert(ret == 0);
				} else {
			...
			if (0 == strcmp(seg_usage->up_sname, EMPTY_UP_SNAPSHOT)) {
				g_message("seg no:%d is maybe need to migrate", \
						seg_usage->segno);
				if (seg_usage->alive_block_num > copy_waterlevel) {
					g_message("seg no:%d is need to migrate", \
							seg_usage->segno);
