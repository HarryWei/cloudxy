...
uint32_t __get_cache_free_size(CACHE_CTRL *cctrl){
	//HLOG_DEBUG("--enter fun %s", __func__);
	uint32_t free_block_size = cctrl->cache_size - \
				   g_queue_get_length(cctrl->dirty_block);
...
int get_continues_blocks(CACHE_CTRL *cctrl, GSList **continue_block_list){
	...
	HLOG_DEBUG("--total dirty block:%d,oldest block no:%u--", \
			size, block->block_no);
	*continue_block_list = g_slist_append(*continue_block_list, block);
	...
	while (TRUE) {
		...
		g_assert(block != NULL);
		if (max_block_no + 1 != block->block_no && \
				min_block_no - 1 != block->block_no) {
			//HLOG_DEBUG("--not find continue block no:%llu!--", 
			//block->block_no);
			break;
		}
		if (block->block_no == max_block_no + 1) {
			//HLOG_DEBUG("--find after continue block no:%llu--", 
			//block->block_no);
			max_block_no++;
			*continue_block_list = \
		...
		if (block->block_no == min_block_no - 1) {
			//HLOG_DEBUG("--find before continue block no:%llu--", \
			block->block_no);
			min_block_no--;
			*continue_block_list = \
