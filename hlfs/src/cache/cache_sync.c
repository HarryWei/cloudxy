...
int cache_sync(CACHE_CTRL *cache_ctrl)
{
	...
	//while ((free_size = __get_cache_free_size(cache_ctrl)) != \
	cache_ctrl->cache_size) {
		while ((free_size = get_cache_free_size(cache_ctrl)) != \
				cache_ctrl->cache_size) {
			HLOG_DEBUG("--sync all block--:free size:%d,total size:%u", \
					free_size, cache_ctrl->cache_size);
