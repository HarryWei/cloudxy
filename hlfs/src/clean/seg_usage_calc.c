...
int seg_usage_calc(struct back_storage *storage,uint32_t block_size, \
		uint32_t is_compress, uint32_t segno, struct inode *refer_inode, \
		SEG_USAGE_T *seg_usage)
{
	...
	HLOG_DEBUG("seg usage's segno:%u,timestamp:%llu", seg_usage->segno, \
			seg_usage->timestamp);
	...
	while (offset < size) {
#if 0
		ret=storage->bs_file_pread(storage, file, (char*)&lh, \
				LOG_HEADER_LENGTH, offset) ;
		//TODO read 64M once
		...
		HLOG_DEBUG("lh->db_num:%d,offset:%d", lh->db_num, offset);
		for (i = 0;i < lh->db_num;i++) {
			//HLOG_DEBUG("for db:%llu", lh->start_db_no + i);
			//uint32_t db_mine_storage_addr_offset = offset + \
			LOG_HEADER_LENGTH + i * block_size;
			if (1 == is_compress) {
				HLOG_DEBUG("COMPRESS: _offset:%u, idx:%d, idxno:%d", \
						_offset, i, lh->start_db_no + i);
			...
			uint64_t db_cur_storage_addr = \
						       get_db_storage_addr_in_inode(storage, refer_inode, \
			...
			if (db_mine_storage_addr != db_cur_storage_addr) {
				...
				HLOG_DEBUG("this is used data block :%u", \
						seg_usage->alive_block_num);
			}
			seg_usage->block_num++;
		}
#endif
		//uint32_t alive_blocks = log_usage_calc(storage, \
		latest_inode, &lh, db_mine_storage_addr_segno,offset, block_size);
		if (orgine_alive_block_num == seg_usage->alive_block_num) {
			...
			//seg_usage->alive_log_bitmap[bitmap_idx] &= ~(1 << \
			(log_idx % sizeof(uint64_t)));
			int x = 0;
			g_array_append_val(tmp_bit_array, x);
		}else{
