...
void test_load_seg_usage()
{
	...
	for(i = 0;i < 100;i++) {
		...
		seg_usage.bitmap = g_malloc0((seg_usage.log_num - 1) / \
				sizeof(gint) + 1);
		int i, idx;
		i = 0;
		idx = i / sizeof(gint);
		...
		g_assert(ret > 0);
		printf("textbuf is =:%s\n", textbuf);
		int ret = file_append_contents(fixture.storage, "SEG_USAGE.TXT", \
	...
	ret = load_all_seg_usage(fixture.storage, "SEG_USAGE.TXT", \
			seg_usage_hashtable);
	g_assert(ret == 0);
	GList *seg_list;
...
#if 1
	g_test_add("/misc/test_load_seg_usage", Fixture, NULL, case_setup, \
			test_load_seg_usage, case_teardown);
#endif
#if 0
	g_test_add("/misc/cache_flush_work_2", Fixture, NULL, case_setup, \
			test_case_cache_flush_2, case_teardown);
#endif 
#if 0
	g_test_add("/misc/cache_flush_work_3", Fixture, NULL, case_setup, \
			test_case_cache_flush_3, case_teardown);
#endif
#if 0
	g_test_add("/misc/cache_flush_work_4", Fixture, NULL, case_setup, \
			test_case_cache_flush_4, case_teardown);
#endif
