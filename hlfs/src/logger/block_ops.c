...
int __read_layer_iblock(struct hlfs_ctrl *hctrl, uint32_t dbno, int layerno, \
		char *iblock_buf){
	//return -1;
	...
	HLOG_DEBUG("dbno:%d,layerno:%d,ibno:%d",dbno,layerno,ibno);
	g_assert(ibno >= 0);
...
int __write_layer_iblock(struct hlfs_ctrl *hctrl, uint32_t dbno, int layerno, \
		char *iblock){
	//HLOG_DEBUG("enter func %s", __func__);
	int ret =0;
	if (NULL != hctrl->icache) {
...
static int read_block_raw(struct hlfs_ctrl *ctrl, uint32_t storage_address, \
		char *block_buf)
{
	//HLOG_DEBUG("enter func %s", __func__);
...
typedef int(*READ_BLOCK_FUN)(struct hlfs_ctrl *ctrl, \
		uint64_t storage_address, char *buf);
static int __load_block_by_no(struct hlfs_ctrl *ctrl, uint32_t no, \
		READ_BLOCK_FUN RB_FUN, char *block){
	...
	if (ctrl->cctrl != NULL) {
		...
		if (ret == 0 ) {
			HLOG_DEBUG("succ to get block from db cache!");
			return 0;
		}
		//g_free(*block);
		HLOG_DEBUG("not find in db cache!");
	}
	...
	if (is_db_in_level1_index_range(db_no)) {
		...
		if (0 > read_layer1_iblock(ctrl, db_no, (char *)_ib)) {
			if (0 != RB_FUN(ctrl, ctrl->inode.iblock, (char *)_ib)) {
				HLOG_ERROR("read_block error for iblock_addr:%llu", \
						ctrl->inode.iblock);
				return -1;
			}
		...
		if (0 > read_layer1_iblock(ctrl, db_no, (char *)_ib)) {
			if (0 != RB_FUN(ctrl, ctrl->inode.doubly_iblock, (char *)_ib)) {
				HLOG_ERROR("read_block error for doubly_iblock_addr:%llu",\
		...
		HLOG_DEBUG("inode.triply_iblock :%llu", ctrl->inode.triply_iblock);	
		...
		if (0 > read_layer1_iblock(ctrl, db_no, (char *)_ib)) {	
			if (0 != RB_FUN(ctrl, ctrl->inode.triply_iblock, (char *)_ib)) {
				//HLOG_ERROR("read_block error for triply_iblock_addr:%llu", \
				ctrl->inode.triply_iblock);
				return -1;
			}
			write_layer1_iblock(ctrl, db_no, (char *)_ib);
		}
		...
		int _idx3 = (db_no - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM * \
...
int load_block_by_no(struct hlfs_ctrl *ctrl, uint32_t no, char *block){
	//HLOG_DEBUG("enter func %s", __func__);
	int ret = __load_block_by_no(ctrl, no, (READ_BLOCK_FUN)read_block_raw, \
			block);
	//HLOG_DEBUG("leave func %s", __func__);
	return ret;
}
