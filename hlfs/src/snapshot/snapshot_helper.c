...
int dump_snapshot_delmark(struct back_storage *storage, 
	...
	ret = file_append_contents(storage, snapshot_file, \
			snapshot_delmark_text, len);
	return ret;
}

/* load all snapshot will remove del snapshot and revise relation upname */
static void predicate_same_upname_snapshot(gpointer key, \
...
static void revise_snapshot_relation(GHashTable *ss_hashtable, \
		GList *remove_list){
	int i = 0;
...
static int load_snapshot_from_text(struct snapshot **ss, \
		const char *buf, int *flag)
{
	//HLOG_DEBUG("enter func %s", __func__);
	const char *sep = SS_ITEM_SEP;
...
int load_all_snapshot(struct back_storage *storage, \
		const char* snapshot_file, GHashTable *ss_hashtable)
{
	//HLOG_DEBUG("enter func %s", __func__);
	int ret = 0;
	char *contents = NULL;
	...
	if (ret != 0) {
		HLOG_ERROR("can not read snapshot content, but may be first start, \
			...
			if(to_remove_ss_list!=NULL){
				...
				for(j = 0; j < g_list_length(to_remove_ss_list); j++){
					...
					if (0 == strcmp(ss->sname,ss_name)) {
						to_remove_ss_list = g_list_remove(to_remove_ss_list, \
								ss_name);
						break;
					}
				}
...
static int compare_snapshot(gconstpointer litem, gconstpointer ritem){
	int ret = 0;
...
/*  order by timestamp */
int redump_all_snapshot(struct back_storage *storage, \
		const char *snapshot_file, GHashTable *ss_hashtable){
	//HLOG_DEBUG("enter func %s", __func__);
		...
		const char *ss_name)
{
	...
	if (ret < 0) {
		HLOG_ERROR("load all ss error, but may be first start, \
				not error, check it please");
...
static int load_all_alive_snapshot(struct back_storage *storage, \
		const char* alive_snapshot_file, GList **alive_ss_list)
{
	//HLOG_DEBUG("enter func %s", __func__);
	int ret = 0;
	char *contents = NULL;
...
int find_latest_alive_snapshot_before_time(struct back_storage *storage, \
		const char* snapshot_file, const char *alive_snapshot_file, \
		struct snapshot **ss, uint64_t timestamp){
	//HLOG_DEBUG("enter func %s", __func__);
	if (snapshot_file == NULL || alive_snapshot_file == NULL || \
			storage == NULL) {
		HLOG_ERROR("Parameter error!");
		return -1;
	}  
	...
	ret = load_all_alive_snapshot(storage, \
			alive_snapshot_file, &alive_snapshot_list);
	if (ret != 0) {
		goto out;
	}
	ret = load_all_snapshot(storage, snapshot_file, ss_hashtable);
	...
	//HLOG_DEBUG("99 list length is %d", \
	g_list_length(alive_snapshot_list));
	...
	for (i = g_list_length(alive_snapshot_list) - 1; i >= 0; i--) {
		...
		HLOG_DEBUG("99 i _ss->timestamp is %llu, _ss->sname is %s", \
				_ss->timestamp, _ss->sname);
...
int is_first_start(struct back_storage *storage, 
	...
	if (EHLFS_NOFILE == storage->bs_file_is_exist(storage,snapshot_file) \
			&& EHLFS_NOFILE == storage->bs_file_is_exist(storage, \
				alive_snapshot_file)) {
		HLOG_DEBUG("first start hlfs ...");
		return HLFS_FS;
	}
	if (EHLFS_NOFILE != storage->bs_file_is_exist(storage, \
				snapshot_file) && EHLFS_NOFILE == \
			storage->bs_file_is_exist(storage, alive_snapshot_file)) {
...
int find_latest_alive_snapshot(struct back_storage *storage, \
		const char* alive_snapshot_file, const char* snapshot_file, \
		struct snapshot **ss) {
	return find_latest_alive_snapshot_before_time(storage, \
			alive_snapshot_file, snapshot_file, ss, G_MAXUINT64);
}

int dump_alive_snapshot(struct back_storage* storage, \
		const char *alive_snapshot_file, struct snapshot *snapshot){
