...
int hlfs_write(struct hlfs_ctrl *ctrl, char *write_buf, \
		uint32_t write_len, uint64_t pos)
{
	//HLOG_DEBUG("enter func %s", __func__);
	if ((NULL == ctrl) || (NULL == write_buf) || \
			(0 == write_len) || (ctrl->sb.seg_size < write_len)) {
		HLOG_ERROR("Params Error");
		return -1;
	}
	HLOG_INFO("Hlfs Write Req pos:%llu, read_len:%d, \
			last_segno:%d, last_offset:%d, cur_file_len:%llu",
			pos,
			write_len,
			ctrl->last_segno,
			ctrl->last_offset,
	...
	if (ctrl->sb.max_fs_size * 1024 * 1024 < pos+write_len) {
		HLOG_ERROR("your config only allow write beyond :%llu", \
				ctrl->sb.max_fs_size);
		//g_mutex_unlock (ctrl->hlfs_access_mutex);
		return -1;
	}		
	...
	if (db_start == db_end && (db_start % BLOCKSIZE != 0 || \
				db_end % BLOCKSIZE != 0) ) {
		HLOG_DEBUG("only need to write part in one block:%llu", \
				pos / BLOCKSIZE);
		...
		//g_mutex_unlock (ctrl->hlfs_access_mutex);
		if ( 1==ret ) {
			//HLOG_DEBUG("fail to load block for addr!\
			%llu for not write yet", pos);
...
write_log:;
	  ...
	  if (ctrl->cctrl != NULL) {
		  ...
		  int ret = dbcache_insert_blocks(ctrl->cctrl, db_start, \
				  (db_end - db_start + 1), datablocks);
		  g_assert(ret == 0);
	  } else {
	  ...
	  //struct write_req * w_req = (struct write_req *)g_malloc0 \
	  ...
	  struct write_rsp * w_rsp = (struct write_rsp*)g_async_queue_pop \
				     (ctrl->write_rsp_aqueue);
	  ret = w_rsp->res;
	  int size = w_rsp->size;
	  //g_free(w_rsp);
	  if (ret != 0) {
