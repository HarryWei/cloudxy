...
static GOptionEntry entries[] = {
	{"filesystem father loc with snapshot",'f', 0, \
		G_OPTION_ARG_STRING,&father_uri_with_snapshot, \
			"father storage uri with snapshot", "SFSLOC"},
	{"filesystem self   loc",'s', 0, G_OPTION_ARG_STRING,&son_uri, \
		"son storage uri", "SFSLOC"},
...
int main(int argc, char *argv[]){
	...
	g_message("father_uri_with_snapshot:%s; son_uri:%s", \
			father_uri_with_snapshot, son_uri);
	gchar **v = NULL;
	...
	if ((0 != son_storage->bs_file_is_exist(son_storage, NULL)) || \
			(0!=son_storage->bs_file_is_exist(son_storage, "superblock"))) {
	...
	if (FALSE == g_key_file_load_from_data(sb_keyfile, \
				content, size, G_KEY_FILE_NONE, NULL)) {
	...
	if (0 != get_cur_latest_segment_info(father_storage, &segno, &offset)) {
		...
		if (segno == 0 && offset == 0) {
			...
			if ( 0 != load_latest_inode_map_entry(father_storage, \
						segno, offset, &imap_entry)) {
	...
	if (0 > load_snapshot_by_name(father_storage, SNAPSHOT_FILE, &ss, \
				father_snapshot)) {
		g_message("load uri:%s ss by name:%s error", \
				father_uri, father_snapshot);
		g_free(ss);
		return -1;
	}
	...
	if (0 != read_fs_meta(father_storage, &father_seg_size, \
				&father_block_size, &father_max_fs_size, &father_is_compress)) {
	...
	uint32_t son_seg_size =  g_key_file_get_integer(sb_keyfile, \
			"METADATA", "segment_size", NULL); 	
	uint32_t son_is_compress =  g_key_file_get_integer(sb_keyfile, \
			"METADATA", "is_compress", NULL);
	if (son_block_size != father_block_size || father_seg_size != \
			son_seg_size || father_is_compress != son_is_compress) {
		g_message("sorry , now father segsize block size compress flag \
				must same as son!!!");
		return -1;	
	}		
	...
	if (0 != file_append_contents(son_storage, "superblock", \
