...
struct back_storage *init_storage_handler(const char *uri)
{
	...
	HLOG_DEBUG("uri:%s, head:%s, dir:%s, fsname:%s, hostname:%s, \
			port:%d, user:%s",
			storage->uri,
			storage->head,
			storage->dir,
...
uint64_t get_last_inode_storage_addr_in_seg( struct back_storage *storage, uint32_t segno) 
{
	...
	if (0 == storage->bs_file_is_exist(storage, segfile)) {
		if (NULL == (file = storage->bs_file_open(storage, \
						segfile, BS_READONLY))) {
		...
		if (sizeof(struct inode_map_entry) != \
				storage->bs_file_pread(storage, file, 	(char*)&im, \
					sizeof(struct inode_map_entry) , info->size - \
	...
	HLOG_DEBUG("im.inode_no:%lld,im.inode_addr:%lld(segno:%u,offset:%u)", \
			im.inode_no,im.inode_addr,get_segno(im.inode_addr), \
			get_offset(im.inode_addr));
	HLOG_DEBUG("leave func %s", __func__);
...
static int restore_last_segno_file(const char *uri, const char *path){
	...
	ret = parse_from_uri(uri, (char **) &head, (char **) &hostname, \
			(char **) &dir, (char **) &fs_name, &port);
	if (ret != 0) {
	...
	sprintf((char *) tmp_segno_file_path,"%s%s", \
			full_segno_file_path,".bak");
	HLOG_DEBUG("full segno file path :%s", full_segno_file_path);
	const char cmd[256];
	pid_t status;
	memset((char *) cmd, 0, 256);
	sprintf((char *) cmd,"%s%s%s%s%s", "hadoop fs", " -cp ", \
			full_segno_file_path," ", tmp_segno_file_path);
	HLOG_DEBUG("cmd is  :%s", cmd);
	status = system(cmd);
	memset((char *) cmd, 0, 256);
	sprintf((char *) cmd, "%s%s%s", "hadoop fs", " -rm ", \
			full_segno_file_path);
	HLOG_DEBUG("cmd is  :%s", cmd);
	status = system(cmd);
	memset((char *) cmd, 0, 256);
	sprintf((char *) cmd, "%s%s%s%s%s", "hadoop fs", " -mv ", \
			tmp_segno_file_path," ", full_segno_file_path);
	HLOG_DEBUG("cmd is  :%s", cmd);
	status = system(cmd);
...
int get_cur_latest_segment_info(struct back_storage *storage, \
		uint32_t *segno,uint32_t *offset){
	HLOG_DEBUG("enter func %s", __func__);
	int ret =0;
		...
		if (g_str_has_suffix(info->name, "seg")) {
			if(latest_st_mtime < info->lmtime){
				...
				if (info->size == 0) {
					...
					g_assert(0 == ret);
					bs_file_info_t * _info = \
								 storage->bs_file_info(storage, info->name);
					*offset = _info->size;
					free(_info);
				}
#endif
	...
	if (latest_file != NULL) {
		...
		*segno = atol(v[0]);
		...
		if (__info->size == 0) {
			...
			g_assert(0==ret);
			bs_file_info_t * _info = storage->bs_file_info(storage, \
					latest_file);
			*offset = _info->size;
			free(_info);
		}else{
 ...
 */
struct inode *load_inode(struct back_storage *storage, \
		uint64_t inode_storage_addr)
{
	HLOG_DEBUG("enter func %s", __func__);
	bs_file_t file = NULL;
	...
	if (0 == storage->bs_file_is_exist(storage, segfile)) {
		...
		if (sizeof(struct inode) != storage->bs_file_pread(storage, \
					file, (char *)my_inode, sizeof(struct inode), offset)) {
			g_free(my_inode);
			storage->bs_file_close(storage, file);
			HLOG_ERROR("pread error!");
			return NULL;
...
struct inode *load_latest_inode(struct back_storage *storage) 
{
	...
	uint64_t inode_storage_addr = \
				      get_last_inode_storage_addr_in_seg(storage, segno);
		...
		uint32_t is_compress)
{
	...
	if (is_db_in_level1_index_range(db_no)) {
		...
		if (1 == is_compress) {
			if ( 0 != read_zblock(storage, inode->iblock, \
						BLOCKSIZE, (char *)ib)) {
				return -1;
			}
		}else{
			if ( 0!= read_block(storage, inode->iblock, \
						BLOCKSIZE, (char *)ib)) {
				return -1;
			}
		}
		...
		uint64_t *ib = (uint64_t *)alloca(BLOCKSIZE);
		if (1 == is_compress) {
			if( 0 != read_zblock(storage, inode->doubly_iblock, \
						BLOCKSIZE,(char *)ib)){
				return -1;
			}
		}else{
			if ( 0 != read_block(storage, \
						inode->doubly_iblock, BLOCKSIZE, (char *)ib)) {
				return -1;
			}
		}
		...
		if (1 == is_compress) {
			if ( 0 != read_zblock(storage, *(ib + idx), \
						BLOCKSIZE, (char *)ib2)) {
				return -1;
			}
		}else{
			if ( 0 != read_block(storage, *(ib + idx), \
						BLOCKSIZE, (char *)ib2)) {
				return -1;
			}
		}
		...
		uint64_t *ib = (uint64_t*)alloca(BLOCKSIZE);
		if (1 == is_compress) {
			if ( 0 != read_zblock(storage, \
						inode->triply_iblock, BLOCKSIZE, (char *)ib)) {
				return -1;
			}
		}else{
			if ( 0 !=read_block(storage, \
		...
		uint64_t idx2 = (db_no - 12 - IB_ENTRY_NUM - \
		...
		if (1 == is_compress) {
			if (0 != read_zblock(storage, *(ib2 + idx2), \
						BLOCKSIZE,(char *)ib3)) {
				return -1;
			}
		}else{
			if (0 != read_block(storage, *(ib2 + idx2), 
						BLOCKSIZE, (char *)ib3)){
				return -1;
			}
		}

		uint64_t idx3 = (db_no - 12 - IB_ENTRY_NUM - \
				IB_ENTRY_NUM * IB_ENTRY_NUM) % IB_ENTRY_NUM;
...
static GKeyFile *get_superblock_keyfile(struct back_storage *storage){
	...
	bs_file_t file = storage->bs_file_open(storage, "superblock", \
			BS_READONLY);
	if (file == NULL) {
	...
	if (FALSE == g_key_file_load_from_data(sb_keyfile, key_file_buf, 
				size, G_KEY_FILE_NONE, NULL)) {
...
int  read_fs_meta_all(struct back_storage *storage,uint32_t *segment_size, \
		uint32_t *block_size, uint64_t *max_fs_size, uint32_t *is_compress, \
	...
	gchar * _uri =  g_key_file_get_string(sb_keyfile, \
			"METADATA", "uri", NULL);
	gint32 _seg_size = g_key_file_get_integer(sb_keyfile, \
			"METADATA", "segment_size", NULL);
	gint32 _block_size = g_key_file_get_integer(sb_keyfile, \
			"METADATA", "block_size", NULL);
	gint64 _max_fs_size = g_key_file_get_int64(sb_keyfile, \
			"METADATA", "max_fs_size", NULL); 
	gint32 _is_compress = g_key_file_get_integer(sb_keyfile, \
			"METADATA","is_compress", NULL);
	if (_uri == NULL || _seg_size == 0 || _block_size ==0 || \
			_max_fs_size == 0 || (_is_compress != 0 && _is_compress!=1)) {
	...
	*is_compress = _is_compress;
	...
	g_free(_uri);
	gchar * _father_uri =  g_key_file_get_string(sb_keyfile, \
			"METADATA", "father_uri", NULL);
	gint64  _snapshot_inode = g_key_file_get_int64(sb_keyfile, \
			"METADATA", "snapshot_inode",NULL);
	gint32  _from_segno = g_key_file_get_integer(sb_keyfile, \
			"METADATA","from_segno", NULL);
	if (_father_uri != NULL) {
...
int read_fs_meta(struct back_storage *storage,uint32_t *segment_size, \
		uint32_t *block_size, uint64_t *max_fs_size, uint32_t *is_compress)
{
	int ret = 0;
	GKeyFile *sb_keyfile = get_superblock_keyfile(storage);
	...
	*is_compress = _is_compress;
	...
	SEGMENT_SIZE = _seg_size;
	HBLOCK_SIZE =_block_size;
	HLOG_DEBUG("SEGMENT_SIZE:%llu,HBLOCK_SIZE:%u", \
			SEGMENT_SIZE, HBLOCK_SIZE);
#endif
out:
	g_key_file_free(sb_keyfile);
	HLOG_DEBUG("leave func %s", __func__);
...
int load_latest_inode_map_entry(struct back_storage *storage, \
		const uint32_t segno, const uint32_t last_offset, struct inode_map_entry *ime)
{
	HLOG_DEBUG("enter func %s", __func__);
	HLOG_DEBUG("777 dbg last offset is %u, seg no is %u", \
			last_offset, segno);
	int ret = 0;
	...
	uint64_t inode_map_entry_pos = (uint64_t) last_offset - \
				       sizeof(struct inode_map_entry);
	HLOG_DEBUG("777 dbg inode map entry pos %llu", inode_map_entry_pos);
	if (sizeof(struct inode_map_entry) != storage->bs_file_pread(storage, \
				file,(char *)ime, sizeof(struct inode_map_entry), \
				inode_map_entry_pos)) {
...
int file_append_contents(struct back_storage *storage, \
		const char* filename, const char *contents, uint32_t size){
	HLOG_DEBUG("enter func %s", __func__);
...
int file_get_contents(struct back_storage *storage, const char *filename, \
	...
	if ( 0!= storage->bs_file_is_exist(storage, filename)) {
		HLOG_ERROR("file is not exist, but may be first start, \
				not error, check it please");
		return -1;
	}
