...
{
	g_print("test 1 >>> \n");
	const char *uri = "local:///tmp/index1env";
	const char *fs_name = "index1fs";
...
/*
	...
	for (i = 0; i < 1024 * 8 * 5; i++)
		content1[i] = 0x01;
	ret = hlfs_write(ctrl, content1, 1024 * 8 * 5, 0);
	g_print("test hlfs read >>> \n");
	ret = hlfs_read(ctrl, content1, 1024 * 40, 0);
	g_assert(ret > 0);
...
*/
	...
	for (i = 0; i < 1024 * 8 * 5 + 512; i++)
		content1[i] = 0x01;
	ret = hlfs_write(ctrl, content1, 1024lu * 8lu * 5lu + 512lu, 0llu);
	g_print("test hlfs read >>> \n");
	ret = hlfs_read(ctrl, content1, 1024lu * 40lu + 512lu, 0llu);
	g_assert(ret > 0);
