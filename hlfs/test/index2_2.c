 ...
 * read the position of 12*BLOCKSIZE+10  datas.
 * Check: read the position of  12*BLOCKSIZE+10  with the length of 5b, check
...
{	
	...
	g_assert(ret == 0);
	ret = hlfs_write(ctrl, content2, 5lu, 12llu * 8llu * 1024llu + 10llu);//begin from the first of second inode
	g_assert(ret>0);
	g_print("test hlfs read >>> \n");
	ret = hlfs_read(ctrl, content2, 5lu, 12llu * 8llu * 1024llu + 10llu);
	g_assert(ret > 0);
	for (i = 0; i < 5; i++) {
