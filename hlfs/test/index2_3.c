...
int main(void)
{
	...
	ret = hlfs_write(ctrl, content2, 16lu * 1024lu + 20lu, 12llu * 8llu * 1024llu);
	g_assert(ret>0);
	g_print("test hlfs read >>> \n");
	ret = hlfs_read(ctrl, content, 10lu, 12llu * 8llu * 1024llu);
	g_assert(ret > 0);
	for (i = 0; i < 10; i++) {
                g_assert(content[i] == 'a');
        }

	ret = hlfs_read(ctrl,content,10,13*8*1024);
	g_assert(ret>0);
	for (i = 0; i < 10; i++) {
                g_assert(content[i] == 'a');
        }
	ret = hlfs_read(ctrl,content,10,14*8*1024+2);
	g_assert(ret>0);
	for (i = 0; i < 10; i++) {
		g_assert(content[i] == 'a');
	}
