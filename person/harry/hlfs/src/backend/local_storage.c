...
int local_file_append(struct back_storage *storage, \
		bs_file_t file, const char *write_buff, uint32_t write_len){
	//HLOG_DEBUG("local -- enter func %s", __func__);
	int ret = 0;
	//HLOG_DEBUG("storage:%p file:%p write_buf:%p write_len:%u", \
	storage, file, write_buff, write_len);
...
bs_file_info_t*
local_list_dir(struct back_storage *storage,\
