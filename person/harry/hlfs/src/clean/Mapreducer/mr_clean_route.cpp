...
class SegUsageCalcMap: public HadoopPipes::Mapper {
	public:
		...
		SegUsageCalcMap(HadoopPipes::TaskContext & context) {
			...
			//HADOOP_ASSERT(m_latest_inode != NULL, \
			"failed to load latest inode ");
			m_block_size = block_size;
			m_is_compress = is_compress;
		}
		...
		void map(HadoopPipes::MapContext& context) {
			...
			char *up_sname;
			ret = get_refer_inode_between_snapshots(m_storage, \
					segno, ss_list, &inode, &up_sname);
			...
			if (ret == 0) {
				...
				ret = seg_usage_calc(m_storage, m_block_size, 
						m_is_compress, segno, inode, &seg_usage);
			...
			if (ret == 2) {
				...
				strncpy(seg_usage.up_sname, EMPTY_UP_SNAPSHOT,
						strlen(EMPTY_UP_SNAPSHOT));
				printf("DBG:--up sname is:%s\n", seg_usage.up_sname);
				inode = load_latest_inode(m_storage);
				ret = seg_usage_calc(m_storage, m_block_size, segno, 
						inode, &seg_usage);
				g_assert(ret == 0);
			}
#if 1
...
class SegUsageCalcReader: public HadoopPipes::RecordReader {
	...
	public:
		...
		SegUsageCalcReader(HadoopPipes::MapContext& context) {
			...
			/* FIXIT : hardcore for get segfile name from 
			 * hadoop proctocol ? */
			...
			printf("GDB:-- filename :%s sizeof:%d\n", _filename.c_str(),
					_filename.size());
			uint64_t _offset = \
					   *(int64_t*) (context.getInputSplit().data() + 2
							   + _filename.size());
			uint64_t offset = Swap64(_offset);
			uint64_t _len = *(int64_t*) (context.getInputSplit().data() + 2
					+ _filename.size() + 8);
...
class SegUsageCalcWriter: public HadoopPipes::RecordWriter {
	...
	public:
		...
		void emit(const std::string& key, const std::string & value) {
			...
			int ret = file_append_contents(m_storage, SEGMENTS_USAGE_FILE,
					segtextbuf, len);
			g_assert(ret == 0);
		}
};
...
int main(int argc, char *argv[]){
	return HadoopPipes::runTask(
			HadoopPipes::TemplateFactory<SegUsageCalcMap, SegUsageCalcReduce,
			void, void, SegUsageCalcReader, SegUsageCalcWriter>());
}
