...
SEG_USAGE_T *load_seg_usage_by_order (struct back_storage *storage, \
	...
	ret = load_all_seg_usage(storage, seg_usage_file, \
			seg_usage_hashtable);
	GList *seg_list;
	...
	int i;
	for (i = 0;i < size;i++) {
		SEG_USAGE_T *_seg_usage = \
					  (SEG_USAGE_T *)g_list_nth_data(seg_list, i);
		memcpy(seg_usages_buf + i * sizeof(SEG_USAGE_T), \
				_seg_usage, sizeof(SEG_USAGE_T));
		g_free(_seg_usage);
	}
