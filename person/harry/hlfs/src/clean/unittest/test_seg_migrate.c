...
void test_seg_migrate()
{
	...
	for (i = 0;i <= max_seg;i++) {
		...
		if (ret == 0) {
			...
			ret = seg_usage_calc(storage, block_size, \
					fixture.hctrl->is_compress, i, inode, &seg_usage);
			g_assert(ret ==0);
			char textbuf[4096];
		...
		if (ret == 2) {
			...
			memset(&seg_usage, 0, sizeof(SEG_USAGE_T));
			ret = seg_usage_calc(storage, block_size, \
					fixture.hctrl->is_compress, i, &fixture.hctrl->inode, &seg_usage);
			g_assert(ret == 0);
			char textbuf[4096];
			memset(textbuf, 4096, 0);
			ret = seg_usage2text(&seg_usage, textbuf);
...
#if 1
	g_test_add("/misc/test_seg_migrate", Fixture, NULL, case_setup, \
			test_seg_migrate, case_teardown);
#endif
#if 0
