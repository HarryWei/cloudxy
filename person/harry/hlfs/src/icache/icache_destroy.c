...
int icache_destroy(ICACHE_CTRL *icache_ctrl){
	...
	if (icache_ctrl->iblock_lru) {
		while(!g_queue_is_empty(icache_ctrl->iblock_lru)){
			iblock_t *iblock = \
					   (gpointer)g_queue_pop_head(icache_ctrl->iblock_lru);
	...
	if (icache_ctrl->iblock_cache) {
		...
		while (g_trash_stack_height(&icache_ctrl->iblock_cache) != 0) {
			HLOG_DEBUG("g_trash_stack_height: %d", \
					g_trash_stack_height(&icache_ctrl->iblock_cache));
			iblock_t *_iblock = \
					    (iblock_t *)g_trash_stack_pop(&icache_ctrl->iblock_cache);
