...
extern "C" {
	...
	int  read_fs_meta_all(struct back_storage *storage, \
			uint32_t *segment_size, uint32_t *block_size,uint64_t *max_fs_size, \
			uint32_t *is_compress, gchar **father_uri,uint64_t *base_father_inode,\
			uint32_t *from_segno);
	uint64_t get_last_inode_storage_addr_in_seg( struct back_storage *storage,\
			uint32_t segno);
	...
	int file_get_contents(struct back_storage *storage, const char *filename, \
			char **constents, uint32_t *size);
	int file_append_contents(struct back_storage *storage, const char *filename, \
			const char *contents, uint32_t size);
#ifdef __cplusplus 
} 
#endif 
