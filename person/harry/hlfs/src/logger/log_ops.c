...
static int compress_dblocks(char *db_buff, uint32_t db_num, \
		uint32_t block_size, char *dzb_buff, uint32_t *real_compressed_size){
	int i = 0;
	char *cur_db = db_buff;
	int offset = 0;
	for (i = 0;i < db_num;i++) {
		...
		g_assert(snappy_compress(cur_db, block_size, dzb_buff + \
					offset + sizeof(uint32_t), &output_length) == SNAPPY_OK);
...
static int update_inode_index(struct inode *inode, \
		struct log_header *log, uint32_t last_segno, \
		uint32_t last_offset, uint32_t block_size){
	...
	if (log->cflag == 1) {
		int i;
		for (i = 0;i < db_num;i++) {
			db_offset += get_zblock_size((char*)log + db_offset) + \
				     sizeof(uint32_t);
			HLOG_DEBUG(" COMPRESSED : db_offset:%d", db_offset);
		}
		ib_offset = db_offset;
		HLOG_DEBUG(" COMPRESSED : ib_offset:%d db_offset:%d", \
				ib_offset, db_offset);
	...
	for (db_cur_no = start_db; db_cur_no <= end_db; db_cur_no++) {
		if (is_db_in_level1_index_range(db_cur_no)) {
			HLOG_DEBUG(" is level1 -- db_cur_no:%d db_offset:%d",\
					db_cur_no, db_offset);
			...
			HLOG_DEBUG("-----dbno:%d,idx:%d, blocks:%llu,db_offset:%d", \
					db_cur_no, _idx,inode->blocks[_idx], db_offset);
			if (log->cflag == 0) {
				db_offset += BLOCKSIZE;
			}else{
				db_offset += get_zblock_size((char*)log + db_offset) \
			...
			if ((db_cur_no - 12 -IB_ENTRY_NUM + 1) % \
					(IB_ENTRY_NUM * IB_ENTRY_NUM) == 0 || db_cur_no == end_db) {
				...
				if (log->cflag == 0) {
					...
					ib_offset += get_zblock_size((char *)log + ib_offset) + \
						     sizeof(uint32_t);
				}
				HLOG_DEBUG("COMPRESSED inode->doubly_iblock:%llu", \
						inode->doubly_iblock);
			}
		}else if (is_db_in_level4_index_range(db_cur_no)) {
			if ((db_cur_no - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM * \
						IB_ENTRY_NUM + 1) % IB_ENTRY_NUM == 0 || db_cur_no == end_db){
				HLOG_DEBUG(" save ib3 ? ");
				if (log->cflag == 0) {
					ib_offset += BLOCKSIZE;
				}else{
			...
			if ((db_cur_no - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM \
						* IB_ENTRY_NUM + 1) % (IB_ENTRY_NUM * IB_ENTRY_NUM)  == \
					0 || db_cur_no == end_db) {
				HLOG_DEBUG(" save ib2 ? ");
				if (log->cflag == 0) {
					ib_offset += BLOCKSIZE;
...
static int update_icache(struct icache_ctrl *icctrl, char *log_iblock_buf, \
		uint32_t db_start_no, uint32_t db_num){
	//HLOG_DEBUG("enter func %s", __func__);
	if (NULL == icctrl) {
	...
	for (i = db_start_no;i < db_start_no + db_num;i++) {
		if (is_db_in_level1_index_range(i)) {
			...
			if ((i - 12 + 1) % IB_ENTRY_NUM == 0 || i == db_start_no+db_num-1) {
				...
				//HLOG_DEBUG("dbno:%d,ibno:%d, offset:%d, log_iblock:% \
				p", i, ibno, offset, log_iblock_buf);
				int  _idx = (i - 12) % IB_ENTRY_NUM;
				uint64_t * ib=(uint64_t *)(log_iblock_buf + offset);
				uint64_t storage_address = *(ib +_idx);
				//HLOG_DEBUG("dbno:%d,ibno:%d, idx:%d, storage_addr:\
				%llu", i, ibno, _idx, storage_address);
				g_assert(ibno >= 0);

				ret = icache_insert_iblock(icctrl, ibno, \
						(char *)log_iblock_buf + offset);
				g_assert(ret == 0);
				offset += BLOCKSIZE;
			}   
		}else if (is_db_in_level3_index_range(i)) {
			if ((i -12 - IB_ENTRY_NUM + 1) % IB_ENTRY_NUM == 0 || \
					i == db_start_no+db_num-1){
				int ibno2 = get_layer2_ibno(i);
				//HLOG_DEBUG("dbno:%d,ibno2:%d", i, ibno2);
				g_assert(ibno2 > 0);	
				ret = icache_insert_iblock(icctrl, ibno2, \
						(char *)log_iblock_buf + offset);
				g_assert(ret == 0);
				offset += BLOCKSIZE;
			}
			if ((i - 12 - IB_ENTRY_NUM + 1) % \
					(IB_ENTRY_NUM * IB_ENTRY_NUM) == 0 ||  i == db_start_no + db_num - 1){
				int ibno1 = get_layer1_ibno(i);
				//HLOG_DEBUG("ibno1:%d", ibno1);
				g_assert(ibno1 > 0);
				ret = icache_insert_iblock(icctrl, ibno1, \
						(char *)log_iblock_buf + offset);
				g_assert(ret == 0);	
				offset += BLOCKSIZE;
			}
			...
			if ((i - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM * IB_ENTRY_NUM  +  1) \
					% IB_ENTRY_NUM == 0 ||i == db_start_no + db_num - 1) {

				int ibno3 = get_layer3_ibno(i);
				//HLOG_DEBUG("dbno:%d,ibno3:%d", i, ibno3);
				g_assert(ibno3>0);		
				ret = icache_insert_iblock(icctrl, ibno3, \
						(char *)log_iblock_buf + offset);
				g_assert(ret == 0);
				offset += BLOCKSIZE;
			}
			...
			if ((i - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM * IB_ENTRY_NUM + 1) \
					% (IB_ENTRY_NUM * IB_ENTRY_NUM * IB_ENTRY_NUM) == 0 \
					||i == db_start_no + db_num -1) {
...
static int dump_log(struct hlfs_ctrl *ctrl,struct log_header *log){
	...
	int size = ctrl->storage->bs_file_append(ctrl->storage, \
			(bs_file_t)ctrl->last_wsegfile_handler, (char *)log, log->log_size);
	HLOG_DEBUG("write to filewrite size %d  expect size %d", \
			size, log->log_size);
	if (size != log->log_size) {
		HLOG_ERROR("write to seg:%d failed, write size %d  expect size %d # ",
				ctrl->last_segno, size, log->log_size, \
				ctrl->last_wsegfile_handler);
		return -1;
	} 
	if (0 != ctrl->storage->bs_file_flush(ctrl->storage, \
				(bs_file_t)ctrl->last_wsegfile_handler)) {
		...
		if (NULL != ctrl->icache) {
			...
			HLOG_DEBUG("-- db_num:%d, ib_offset:%d,log:%p", \
					log->db_num,ib_offset,(char*)log + ib_offset);
			ret = update_icache(ctrl->icache, 
					(char *)log + ib_offset, log->start_db_no, log->db_num);
			g_assert(ret == 0);
#endif
		}
		//g_mutex_lock (ctrl->hlfs_access_mutex);
		//HLOG_DEBUG("last offset ................... %d",ctrl->last_offset);
		ret = update_inode_index(&ctrl->inode, log, ctrl->last_segno, \
				ctrl->last_offset, ctrl->sb.block_size);
		g_assert(ret == 0);
...
int __append_log(struct hlfs_ctrl *ctrl,char *db_buff, uint32_t db_start, \
		uint32_t db_end, uint32_t no_compressed){
	...
	if (!_is_compressed) {
		...
		uint32_t max_compressed_size = \
					       snappy_max_compressed_length(BLOCKSIZE);
		HLOG_DEBUG("COMPRESSED: max_compressed_size :%d",\
				max_compressed_size);
		log_buff = (char *)g_malloc0((max_compressed_size + \
					sizeof(uint32_t)) * (db_end-db_start + 1) + \
				(max_compressed_size + sizeof(uint32_t)) * \
				ib_amount(db_start, db_end) \
				+ sizeof(struct inode) + sizeof(struct inode_map_entry) + \
				sizeof(struct log_header));
		g_assert(log_buff != NULL);
		char *compressed_db_buff = log_buff + LOG_HEADER_LENGTH;
		uint32_t real_compressed_size = 0;
		int ret = compress_dblocks(db_buff, (db_end-db_start + \
					1), BLOCKSIZE, compressed_db_buff, &real_compressed_size);
		HLOG_DEBUG("COMPRESSED: real_compressed_size :%d", \
	...
	//HLOG_DEBUG(" db_data_len:%d ib_data_len:%d BLOCKSIZE: \
	%d", db_data_len, ib_data_len, BLOCKSIZE);
	db_offset = LOG_HEADER_LENGTH;
	for (db_cur_no = db_start,i = 0; db_cur_no <= db_end; db_cur_no++,i++) {  
		...
		if (is_db_in_level1_index_range(db_cur_no)) {
			...
			//HLOG_DEBUG(" idx:%u",_idx);
#if 0
			set_segno (&ctrl->inode.blocks[_idx], \
					ctrl->last_segno);
			set_offset(&ctrl->inode.blocks[_idx], \
					ctrl->last_offset + db_offset);
#endif
			/*memcpy(cur_log_buff_ptr,cur_block_ptr, BLOCKSIZE);*/
			if (_is_compressed) {
				db_offset += get_zblock_size(cur_log_buff_ptr) \
					     + sizeof(uint32_t);
			}else{
				db_offset += BLOCKSIZE;
			}
			...
			HLOG_DEBUG("is level2 -- db_cur_no:%d db_offset:%d", \
					db_cur_no, db_offset);
			...
			HLOG_DEBUG("-- cur_dbno:%d,idx:%d,storage address:%llu", \
					db_cur_no,_idx, *(ib1 + _idx));
			/*memcpy(cur_log_buff_ptr,cur_block_ptr,BLOCKSIZE);*/
			if ((db_cur_no - 12 + 1) % IB_ENTRY_NUM == 0 || db_cur_no == db_end ) {
				...
				if (_is_compressed) {
					...
					g_assert(snappy_compress((const char *)ib1, BLOCKSIZE, 
								ib1_buff, &output_length) == SNAPPY_OK);
					ib1_buff_size = output_length;
					HLOG_DEBUG("COMPRESSED: after compress ib1_buff_size %d", \
							ib1_buff_size);
					memcpy((char *)log_buff + ib_offset + sizeof(uint32_t), \
							(char *)ib1_buff, ib1_buff_size);  
					*(uint32_t *)((char *)log_buff + ib_offset) = ib1_buff_size;
					ib_offset += ib1_buff_size + sizeof(uint32_t);
				}else{
					ib1_buff = (char *)ib1;
					ib1_buff_size = BLOCKSIZE;
					memcpy((char *)log_buff + ib_offset , \
							(char *)ib1_buff, ib1_buff_size);  
			...
			HLOG_DEBUG(" is level3 -- db_cur_no:%d db_offset:%d", \
					db_cur_no,db_offset);
			//uint64_t *_ib = NULL;
			if (TRUE == ib1_need_load && ctrl->inode.doubly_iblock == 0) {
				...
				memset(ib1, 0, BLOCKSIZE);
				ib1_need_load = FALSE;
			}else if(TRUE == ib1_need_load && \
					ctrl->inode.doubly_iblock != 0){
				if (0 > read_layer1_iblock(ctrl, db_cur_no, ib1)) { 
					HLOG_DEBUG("COMPRESSED: ctrl->inode.doubly_iblock:%llu",
							ctrl->inode.doubly_iblock);
					if (0 != read_block_fast(ctrl, \
								ctrl->inode.doubly_iblock, ib1)) {
						HLOG_ERROR("allocate error!");
						g_assert(0);
						return -1;
					}
			...
			HLOG_DEBUG("cur_dbno:%u,idx:%u, *(ib1 + _idx):%llu",\
					db_cur_no, _idx, *(ib1 + _idx));
			//uint64_t *_ib2 = NULL; 
			...
			//memcpy(cur_log_buff_ptr, cur_block_ptr,BLOCKSIZE);
			//HLOG_DEBUG("cur_dbno:%u, _idx2:%u, \
			*(ib2+_idx2):%llu", db_cur_no, _idx2,*(ib2 + _idx2));


				      if ((db_cur_no -12 - IB_ENTRY_NUM + 1) % IB_ENTRY_NUM == \
						      0 || db_cur_no == db_end) {
					      //HLOG_DEBUG(" save ib2");
					      set_segno ((ib1 + _idx), ctrl->last_segno);
					      ...
					      if (_is_compressed) {
						      ...
						      g_assert(snappy_compress((const char *)ib2, \
									      BLOCKSIZE, ib2_buff, &output_length) == SNAPPY_OK);
						      ib2_buff_size = output_length;
						      HLOG_DEBUG("COMPRESSED: after compress ib2_buff_size %d", \
								      ib2_buff_size);
						      memcpy((char *)log_buff + ib_offset + sizeof(uint32_t), \
								      (char *)ib2_buff, ib2_buff_size);  
						      //ib_offset += BLOCKSIZE;
				      ...
				      if ((db_cur_no - 12 -IB_ENTRY_NUM + 1) % \
						      (IB_ENTRY_NUM * IB_ENTRY_NUM) == 0 || db_cur_no == db_end) {
					      //HLOG_DEBUG(" save ib1");
#if 0
					      set_segno (&ctrl->inode.doubly_iblock, ctrl->last_segno);
					      set_offset(&ctrl->inode.doubly_iblock, \
							      ctrl->last_offset + ib_offset);
#endif
					      ...
					      if (_is_compressed) {
						      ...
						      ib1_buff = (char *)alloca(output_length);
						      g_assert(ib1_buff != NULL);
						      g_assert(snappy_compress((const char *)ib1, \
									      BLOCKSIZE, ib1_buff, &output_length) == SNAPPY_OK);
						      ib1_buff_size = output_length;
						      HLOG_DEBUG("COMPRESSED: after compress ib1_buff_size %d", \
								      ib1_buff_size);
						      memcpy((char *)log_buff + ib_offset + sizeof(uint32_t),\
			...
			HLOG_DEBUG(" is level4 -- db_cur_no:%d db_offset:%d", \
					db_cur_no, db_offset);
			//uint64_t *_ib = NULL;
			...
			if ((db_cur_no - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM * \
				...
				if (_is_compressed) {
					...
					g_assert(snappy_compress((const char *)ib3, \
								BLOCKSIZE, ib3_buff, &output_length) \
							== SNAPPY_OK);
					ib3_buff_size = output_length;
					HLOG_DEBUG("COMPRESSED: after compress ib3_buff_size %d", \
							ib3_buff_size);
					memcpy((char *)log_buff + ib_offset + sizeof(uint32_t), \
							(char *)ib3_buff, ib3_buff_size);  
					//ib_offset += BLOCKSIZE;
			...
			if ((db_cur_no - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM \
				...
				if (_is_compressed) {
					...
					HLOG_DEBUG("COMPRESSED: after compress ib2_buff_size %d", \
							ib2_buff_size);
					memcpy((char *)log_buff + ib_offset + \
							sizeof(uint32_t), (char *)ib2_buff, ib2_buff_size);  
					*(uint32_t *)((char *)log_buff + ib_offset) \
						= ib2_buff_size;
					//ib_offset += BLOCKSIZE;
					ib_offset += ib2_buff_size + sizeof(uint32_t);
				}else{
			...
			if((db_cur_no - 12 - IB_ENTRY_NUM - IB_ENTRY_NUM * \
						IB_ENTRY_NUM + 1) % (IB_ENTRY_NUM * IB_ENTRY_NUM * \
							IB_ENTRY_NUM) == 0 || db_cur_no == db_end){
#if 0
				set_segno (&ctrl->inode.triply_iblock, ctrl->last_segno);
				set_offset(&ctrl->inode.triply_iblock, \
						ctrl->last_offset + ib_offset);
#endif
				//memcpy((char *)log_buff + ib_offset, (char *)ib1, BLOCKSIZE);
				//ib_offset += BLOCKSIZE;
				...
				if (_is_compressed) {
					...
					HLOG_DEBUG("COMPRESSED: after compress ib1_buff_size %d", \
							ib1_buff_size);
					memcpy((char *)log_buff + ib_offset + \
							sizeof(uint32_t), (char *)ib1_buff, ib1_buff_size);  
					//ib_offset +=BLOCKSIZE;
					*(uint32_t *)((char *)log_buff + ib_offset) \
						= ib1_buff_size;
					ib_offset += ib1_buff_size +  sizeof(uint32_t);
...
__inode_create:;
	       ...
	       HLOG_DEBUG("last offset:%u, last segno:%u log head len:%d iboffset:%d",\
	       ...
	       set_offset(&ctrl->imap_entry.inode_addr, ctrl ->last_offset\
			       + offset);    
	       HLOG_DEBUG("inode address's offset %llu, give it %u", \
			       ctrl->imap_entry.inode_addr, ctrl->last_offset + offset);
#endif 
	       memcpy((char *)log_buff + offset + sizeof(struct inode), \
			       &ctrl->imap_entry, sizeof(struct inode_map_entry));
	       ...
	       //g_assert((ib_offset - db_data_len - LOG_HEADER_LENGTH) % \
	       BLOCKSIZE == 0);
	       ...
	       struct inode _inode;
	       memcpy((char *)&_inode, (char *)&ctrl->inode, \
			       sizeof(struct inode));
	       update_inode_index(&_inode, lh, ctrl->last_segno, \
			       ctrl->last_offset, ctrl->sb.block_size);
	       memcpy((char *)log_buff + offset, (char *)&_inode, \
			       sizeof(struct inode));
#endif

	       HLOG_DEBUG("log size:%d,log header:%d,inode:%d,inode map:%d,dbnum:%d,\
			       ibnum:%d", lh->log_size, sizeof(struct log_header), \
			       sizeof(struct inode), sizeof(struct inode_map_entry), \
			       lh->db_num, lh->ib_num); 
...
int append_log(struct hlfs_ctrl *hctrl, char *db_buff, uint32_t db_start, \
		uint32_t db_end, uint32_t no_compressed){
	guint32 BLOCKSIZE = hctrl->sb.block_size;
	int expand_size = (db_end-db_start + 1) * BLOCKSIZE + \
			  ib_amount(db_start, db_end) * BLOCKSIZE + LOG_HEADER_LENGTH + \
			  sizeof(struct inode) + sizeof(struct inode_map_entry);
	...
	gboolean _is_compressed= (no_compressed == 0) ? \
				 FALSE:hctrl->is_compress;
	if (!_is_compressed) {
		...
		uint32_t max_expand_size = (max_compressed_size + \
				sizeof(uint32_t)) * (db_end - db_start + 1) + \
					   (max_compressed_size + sizeof(uint32_t)) * \
					   ib_amount(db_start, db_end) + sizeof(struct inode) + \
					   sizeof(struct inode_map_entry) + LOG_HEADER_LENGTH;
		if (hctrl->last_offset + max_expand_size > hctrl->sb.seg_size) {
			hctrl->last_segno++;
			hctrl->last_offset = 0;
		}
	}
	//HLOG_DEBUG("last segno:%u last offset:%u", \
	hctrl->last_segno, hctrl->last_offset);
	uint32_t size; 
	size = __append_log(hctrl, db_buff, (uint32_t) db_start, \
			(uint32_t) db_end, no_compressed);
	g_assert(size > 0);
	//HLOG_DEBUG("cur last offset:%d, log size:%d,next last offset:%d", \
	hctrl->last_offset, size, hctrl->last_offset + size);
