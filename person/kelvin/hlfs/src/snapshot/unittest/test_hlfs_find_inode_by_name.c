/*
 *  hlfs/src/snapshot/unittest/test_hlfs_find_inode_by_name.c
 *  
...
test_hlfs_find_inode_by_name(Fixture *fixture, const void *data) {
	...
	ret = hlfs_find_inode_by_name(uri, "T10", &inode_addr);
	g_message("ret is %d", ret);
	g_assert(ret == -1);
	g_message("We can not find T10's inode");
	inode_addr = 0;
	ret = hlfs_find_inode_by_name(uri, "T3", &inode_addr);
	g_assert(ret == 0);
	g_message("T3's inode addr is [%llu]", inode_addr);
	inode_addr = 0;
	ret = hlfs_find_inode_by_name(uri, "T6", &inode_addr);
	g_assert(ret == 0);
	g_message("T6's inode addr is [%llu]", inode_addr);
	g_message("leave func %s", __func__);
	return ;
}
