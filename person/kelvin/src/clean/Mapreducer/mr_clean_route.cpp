...
public:
	...
	void map(HadoopPipes::MapContext& context) {
		...
		char *up_sname;
		ret = get_refer_inode_between_snapshots(m_storage, segno, ss_list,
				&inode, &up_sname);
		...
		if (ret == 0) {
			...
			ret = seg_usage_calc(m_storage, m_block_size,m_is_compress,segno, inode,
					&seg_usage);
		...
		if (ret == 2) {
			...
			ret = seg_usage_calc(m_storage, m_block_size, segno, inode,
					&seg_usage);
			g_assert(ret == 0);
		}
#if 1
...
public:
	...
	void reduce(HadoopPipes::ReduceContext& context) {
		...
		while (context.nextValue()) {
			printf("DBG:--key:%s value%s\n", context.getInputKey().c_str(),
					context.getInputValue().c_str());
		}
...
public:
	...
	SegUsageCalcReader(HadoopPipes::MapContext& context) {
		...
		printf("GDB:-- filename :%s sizeof:%d\n", _filename.c_str(),
				_filename.size());
		uint64_t _offset = *(int64_t*) (context.getInputSplit().data() + 2
				+ _filename.size());
