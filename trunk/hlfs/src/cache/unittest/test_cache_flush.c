 ...
 */
Fixture fixture;
static int test_printf(void *handle, char *buf, uint32_t start_no, \
		uint32_t end_no){
	printf("%d, %d\n", start_no, end_no);
	return 0;
}
void case_setup()
{
	...
	//g_hash_table_insert(fixture.cache_ctrl->block_map, \
	(gpointer)&block->block_no, (gpointer)block);
}

/*
 * case1 test:
 * --call the function hlfs_take_snapshot with the arguments 
 * ctrl & "test_snapshot";
...
#if 1
	g_test_add("/misc/cache_flush_work_1", Fixture, NULL, case_setup, \
			test_case_cache_flush_1, case_teardown);
#endif
#if 1
