...
void case_teardown()
{
	if (fixture.cache_ctrl->block_cache) {
		int i = 0;
		while (g_trash_stack_height(&fixture.cache_ctrl->block_cache) \
				!= 0) {
			g_message("g_trash_stack_height: %d", \
					g_trash_stack_height(&fixture.cache_ctrl->block_cache));
			block_t *_block = \
...
int main(int argc, char **argv){
	...
	g_test_add("/misc/cache_init", Fixture, NULL, case_setup, \
			test_cache_init, case_teardown);
	return g_test_run();
}
