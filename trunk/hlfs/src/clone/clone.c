...
int faimly_init(FAMILY_CTRL *fctrl, char *furi, uint64_t fbase_inode, \
		uint32_t fsegno){
	int ret = 0;
	...
	do {
		...
		//g_tree_insert(fctrl->seg_storage_map, \
		GINT_TO_POINTER((uint32_t)(segno-1), storage);
		...
		fctrl->seg_storage_list = g_list_append(fctrl->seg_storage_list, \
				storage_item);
		if (0 != (ret = read_fs_meta_all(storage, &seg_size, &block_size, \
						&max_fs_size, &is_compress, &father_uri, &base_father_inode, \
						&from_segno))) {
...
struct back_storage *get_parent_storage(FAMILY_CTRL *fctrl, uint32_t segno){
	...
	for (i = g_list_length(fctrl->seg_storage_list)-1; i >= 0; i--) {
		STORAGE_ITEM *storage_item = \
					     g_list_nth_data(fctrl->seg_storage_list, i);
		HLOG_DEBUG("storage_item's segno:%d,search segno:%d", \
				storage_item->segno, segno);
