...
#include "glib.h"
#include "icache.h"
//int icache_insert_blocks(CACHE_CTRL *cache_ctrl, \
...
int icache_insert_iblock(ICACHE_CTRL *icache_ctrl, uint32_t iblock_no, \
	...
	if ((icache_ctrl->icache_size - \
				g_queue_get_length(icache_ctrl->iblock_lru)) < \
			icache_ctrl->invalidate_once_size) {
		...
		while (count --) {
			...
			g_hash_table_remove(icache_ctrl->iblock_map, \
					&(iblock->iblock_no));
			g_trash_stack_push(&icache_ctrl->iblock_cache,iblock);
		}
		HLOG_DEBUG("now has %d iblock", \
				g_queue_get_length(icache_ctrl->iblock_lru));
	}
