/*
 *  hlfs/src/snapshot/unittest/test_hlfs_find_inode_before_time.c
 *  
...
static void 
hlfs_find_inode_before_time_setup(Fixture *fixture, const void *data) {
	...
	gchar *content = g_key_file_to_data(sb_keyfile, \
			NULL, NULL);
	char *sb_file_path = g_build_filename(fs_dir, \
			"superblock", NULL);
...
static void 
hlfs_find_inode_before_time_tear_down(Fixture *fixture, const void *data) {
	const char *test_dir = (const char *) data;
...
int main(int argc, char **argv){
	...
	g_test_add("/misc/hlfs_find_inode_before_time", 
			Fixture, 
			g_get_current_dir(),
			hlfs_find_inode_before_time_setup, 
			test_hlfs_find_inode_before_time, 
			hlfs_find_inode_before_time_tear_down);
	g_message("leave func %s", __func__);
	return g_test_run();
}
