 ...
 */
struct hlfs_ctrl *__init_hlfs(const char *uri, uint32_t is_clean_start, uint32_t seg_clean_check_period, uint32_t copy_waterlevel)
{
	//HLOG_DEBUG("enter func %s", __func__);
	...
	if (ctrl->last_segno != 0 || ctrl->last_offset != 0) {
		...
		if( 0 != load_latest_inode_map_entry(ctrl->storage, \
					ctrl->last_segno, ctrl->last_offset, &ctrl->imap_entry)) {
		...
		if (NULL != ctrl->family) {
			...
			uint32_t last_offset = offset + sizeof(struct inode) + \
					       sizeof(struct inode_map_entry);
			if (0 != load_latest_inode_map_entry(storage, segno, \
						last_offset, &ctrl->imap_entry)) {
				HLOG_ERROR("load inode map entry failed");
				ret = -1;
				goto out;
			...
			HLOG_DEBUG("segno:%d, offset:%d, \
					ctrl->imap_entry->node_addr:%llu", segno, offset, \
	...
	HLOG_INFO("Raw Hlfs Ctrl Init Over ! uri:%s, max_fs_size:%llu, \
			seg_size:%u, block_size:%u, last_segno:%u, last_offset:%u, \
			start_segno:%d, io_nonactive_period:%u",
			uri,
			ctrl->sb.max_fs_size,
			ctrl->sb.seg_size,
			ctrl->sb.block_size,
...
struct hlfs_ctrl *init_hlfs(const char *uri)
{      
	if (NULL == uri) {
		HLOG_ERROR("Params Error");	
		return NULL;
	...
	struct hlfs_ctrl * hlfs_ctrl = __init_hlfs(uri, 1, \
			seg_clean_check_period, seg_clean_check_period);
	if (NULL == hlfs_ctrl) {
		HLOG_ERROR("init raw hlfs ctrl failed");
		return NULL;
	}
#if 1
	uint32_t block_size, cache_size, flush_interval, \
	...
	if (flush_once_size * block_size * 64 > hlfs_ctrl->sb.seg_size) {
		HLOG_ERROR("flush_once_size can not too much:%u", \
				flush_once_size); 
		goto out;
	}
	...
	if (iblock_size != hlfs_ctrl->sb.block_size) {
		HLOG_ERROR("cache block size is not equal to block size \
				in superblock"); 
			goto out;
	}
	...
	ret = icache_init(hlfs_ctrl->icache, iblock_size, icache_size, \
			invalidate_trigger_level, invalidate_once_size);
	if (ret != 0) {
		HLOG_ERROR("init cache failed");
	...
	HLOG_INFO("Indirect  Block Cache Init Over ! icache_size:%u, \
			iblock_size:%u,invalidate_trigger_level:%u, \
			invalidate_once_size:%u", icache_size, iblock_size, \
			invalidate_trigger_level, invalidate_once_size); 
#endif 
	return hlfs_ctrl;
out:
...
struct hlfs_ctrl *init_hlfs_by_config(const char *config_file_path)
{
	...
	gboolean res = g_key_file_load_from_file (hlfs_conf_keyfile, \
			config_file_path, G_KEY_FILE_NONE, NULL);
	if (res == FALSE) {
	...
	const char *uri = g_key_file_get_string (hlfs_conf_keyfile, \
			"STORAGE", "storage_uri", NULL);

	uint64_t seg_clean_check_period = \
					  g_key_file_get_uint64 (hlfs_conf_keyfile, "STORAGE", \
							  "seg_clean_check_period", NULL);
	if (seg_clean_check_period == 0) {
		seg_clean_check_period = DEF_IO_NONACTIVE_PERIOD;
	}

	uint64_t seg_copy_waterlevel = g_key_file_get_uint64 (hlfs_conf_keyfile, \
			"STORAGE", "seg_copy_waterlevel", NULL);
	if (seg_copy_waterlevel == 0) {
		seg_clean_check_period = DEF_SEG_COPY_WATERLEVEL;
	}

	gboolean _is_start_clean = g_key_file_get_boolean (hlfs_conf_keyfile, \
			"STORAGE", "is_start_clean", NULL);
	int is_start_clean;
	...
	struct hlfs_ctrl *hlfs_ctrl = __init_hlfs(uri, is_start_clean, \
			seg_copy_waterlevel, seg_clean_check_period);
	if (hlfs_ctrl == NULL) {
	...
	if (TRUE == g_key_file_has_group(hlfs_conf_keyfile, "CACHE")) {
		//gsize length=0;
		//gchar * keys = g_key_file_get_keys(hlfs_conf_keyfile, \
		"CACHE", &length, NULL);
		gboolean enable = g_key_file_get_boolean (hlfs_conf_keyfile, \
				"CACHE", "is_enable_cache", NULL);
		//HLOG_DEBUG("enable is :%d", enable); 
		if (TRUE ==  enable) {
			HLOG_DEBUG("We Do Support Data Data Cache !!!"); 
			uint32_t block_size, cache_size, flush_interval, \
				flush_trigger_level, flush_once_size;
			block_size 		= g_key_file_get_uint64 (hlfs_conf_keyfile, \
					"CACHE", "block_size", NULL);
			cache_size	= g_key_file_get_uint64 (hlfs_conf_keyfile, "CACHE", \
					"cache_size", NULL);
			flush_interval	= g_key_file_get_uint64(hlfs_conf_keyfile, \
					"CACHE", "flush_interval", NULL);
			flush_trigger_level = g_key_file_get_uint64 (hlfs_conf_keyfile, \
					"CACHE", "flush_trigger_level", NULL);
			flush_once_size	= g_key_file_get_uint64 (hlfs_conf_keyfile, \
					"CACHE", "flush_once_size", NULL);
			/* check .... */
			if (block_size != hlfs_ctrl->sb.block_size) {
				HLOG_ERROR("cache block size is not equal to block \
						size in superblock"); 
					goto out;
			}
			if (flush_trigger_level > 100) {
	...
	if (TRUE == g_key_file_has_group(hlfs_conf_keyfile, "ICACHE")) {
		//gsize length=0;
		//gchar *keys = g_key_file_get_keys(hlfs_conf_keyfile, "CACHE", \
		&length, NULL);
		gboolean enable = g_key_file_get_boolean (hlfs_conf_keyfile, \
				"ICACHE", "is_enable_icache", NULL);
		//HLOG_DEBUG("enable is :%d", enable); 
		if (TRUE ==  enable) {
			HLOG_DEBUG("do support cache!"); 
			uint64_t iblock_size, icache_size, invalidate_trigger_level, \
				invalidate_once_size;
			iblock_size = g_key_file_get_uint64 (hlfs_conf_keyfile, "ICACHE", \
					"iblock_size", NULL);
			icache_size = g_key_file_get_uint64 (hlfs_conf_keyfile, "ICACHE", \
					"icache_size", NULL);
			invalidate_trigger_level = g_key_file_get_uint64 (hlfs_conf_keyfile,\
					"ICACHE", "invalidate_trigger_level", NULL);
			invalidate_once_size = g_key_file_get_uint64 (hlfs_conf_keyfile, 
					"ICACHE", "invalidate_once_size", NULL);
			/* check .... */
			if (iblock_size != hlfs_ctrl->sb.block_size) {
				HLOG_ERROR("cache block size is not equal to block size in \
						superblock"); 
					goto out;
			}
			if (invalidate_once_size > 100) {
			...
			if (invalidate_once_size * iblock_size * 64 > \
					hlfs_ctrl->sb.seg_size) {
				HLOG_ERROR("flush_once_size can not too much:%llu", \
						invalidate_once_size); 
				goto out;
			}
			...
			HLOG_INFO("Indirect  Block Cache Init Over ! icache_size:%llu, \
					iblock_size:%llu, invalidate_trigger_level:%llu, \
					invalidate_once_size:%llu", icache_size, iblock_size, \
					invalidate_trigger_level, invalidate_once_size); 
		}
	}   
	g_key_file_free (hlfs_conf_keyfile);
