...
static GOptionEntry entries[] = {
	{"filesystem location", 'u', 0, G_OPTION_ARG_STRING, &uri, \
		"filesystem storage uri", "FSLOC"},
	//	{"filesystme name", 'f', 0, G_OPTION_ARG_FILENAME, &fsname, \
	"filesystem name", "NAME"},
		    {"filesystem block size", 'b', 0, G_OPTION_ARG_INT, &block_size, \
			    "filesystem block size", "BLOCKSIZE"},
		    {"filesystem segment size", 's', 0, G_OPTION_ARG_INT, &seg_size, \
			    "filesystem segment size", "SEGSIZE"},
		    {"filesystem max size", 'm', 0, G_OPTION_ARG_INT64, &max_fs_size, \
			    "filesystem  max size(M)", "FS MAX SIZE"},
		    {"block compress", 'c', 0, G_OPTION_ARG_INT, &is_compress, \
			    "filesystem  block compress(0/1)", "FS COMPRESS"},
...
int main(int argc, char *argv[])
{
	...
	if ((0 == storage->bs_file_is_exist(storage, NULL)) && \
			(0==storage->bs_file_is_exist(storage, "superblock"))) {
