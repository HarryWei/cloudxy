...
static GOptionEntry entries[] = {
	{"filesystem uri",'u', 0, G_OPTION_ARG_STRING, &uri, \
		"filesystem storage uri", "FSLOC"},
	{"start_segno", 's', 0, G_OPTION_ARG_INT, &start_segno, \
		"start seg no", "START"},
	{"end_segno", 'e', 0, G_OPTION_ARG_INT, &end_segno, \
		"end seg no", "END"},
...
int main(int argc, char *argv[])
{
	...
	g_message("uri is :%s, start segno:%d, end segno:%d", uri, \
	...
	int ret = read_fs_meta(storage, &segment_size, &block_size, \
			&max_fs_size, &is_compress);
	g_message("segment size:%u,block size:%u,max fs size%llu", \
	...
	int i;
	GHashTable *seg_usage_hashtable = g_hash_table_new_full(g_direct_hash, \
	...
	for (i=0; i<num_entries; i++) {
		g_message("file:%s,size:%llu, time:%llu", info->name, \
		...
		if (g_str_has_suffix(info->name, "seg")) {
			...
			gpointer tmp = g_hash_table_lookup(seg_usage_hashtable, \
					GINT_TO_POINTER(segno));
			struct segment_usage *seg_usage;
			if (tmp == NULL) {
				g_message("can not find seg:%u in segment usage file", segno);
				seg_usage = \
					    (struct segment_usage*)g_malloc0(sizeof(struct segment_usage));
...
#endif 
			seg_usage_calc(storage, info->name, latest_inode, \
					seg_usage, block_size);
	...
	for (i = start_segno; i<=end_segno; i++) {
		...
		ret = seg_usage_calc(storage, HBLOCK_SIZE, is_compress, \
				i, inode, &seg_usage);
		if (ret != 0) {
