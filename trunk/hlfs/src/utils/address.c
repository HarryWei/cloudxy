...
{
	uint32_t IB_ENTRY_NUM = HBLOCK_SIZE / sizeof(uint64_t);
	if (db_no < (IB_ENTRY_NUM * IB_ENTRY_NUM + IB_ENTRY_NUM + 12 ) && \
			db_no >= (IB_ENTRY_NUM + 12)) {
		return TRUE;
	} else {
		return FALSE;
	}
...
{
	uint32_t IB_ENTRY_NUM = HBLOCK_SIZE / sizeof(uint64_t);
	if (db_no < (IB_ENTRY_NUM * IB_ENTRY_NUM * IB_ENTRY_NUM + \
				IB_ENTRY_NUM * IB_ENTRY_NUM + IB_ENTRY_NUM + 12) && db_no >= \
			((IB_ENTRY_NUM * IB_ENTRY_NUM) + IB_ENTRY_NUM + 12)) {
		return TRUE;
	} else {
		return FALSE;
	}
